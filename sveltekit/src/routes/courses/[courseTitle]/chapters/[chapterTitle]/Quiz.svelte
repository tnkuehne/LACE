<script lang="ts">
    ...
    function submitAnswer() {
        ...
        isCorrect = JSON.stringify(selectedAnswers.sort()) === JSON.stringify(questions[currentQuestionIndex].correct.sort());

        // If the answer is incorrect, find the incorrect answers
        if (!isCorrect) {
            incorrectAnswers = selectedAnswers.filter(answer => !questions[currentQuestionIndex].correct.includes(answer));
        } else {
            incorrectAnswers = [];
        }
    }
...
<Card.Root class="inline-block">
    ...
    <Card.Content>
        <div class="flex flex-col space-y-2 items-start">
            {#each questions[currentQuestionIndex].answers as {text}, index}
                <Button
